...
('(\{\|\n)(.*[^\}]+)(\|\})',lambda m:'<table>'+'\n'.join([rpn.rpn_row(line) for line in re.split('\|-',m.group(2))])+'</table>'),
('RPN\((.*?)\)', lambda m: rpn.rpn(m.group(1))),('(^|[^=/\-_A-Za-z0-9?])@(\w+)',lambda m: h+w+m.group(2)+'&amp;q=f>@'+m.group(2)+'</a>'),
...
main=lambda f=f:`(os.getenv("REQUEST_METHOD")!="POST") or not edit or ('t' in f or (os.remove('w/'+y) and False))\
    and open('w/'+y,'w').write(f['t'][0])`+`sys.stdout.write("Content-type: text/html; charset=utf-8\r\n\r\n"\
        ...
        <script type="text/javascript" src="../ASCIIMathML.js"></script></head><title>%s</title>'%y+
